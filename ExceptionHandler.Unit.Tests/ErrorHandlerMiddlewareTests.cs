...
using YuraSoft.ErrorHandler.Middleware;

namespace YuraSoft.ErrorHandler.Unit.Tests
{
	public class ErrorHandlerMiddlewareTests
	{
		...
		public async Task InvokeAsync_ShouldBeOk_WhenRequestDelegateWriteResponseWithBody()
		{
			...
			Mock<IErrorHandler> errorHandlerMock = new Mock<IErrorHandler>();
			errorHandlerMock.Setup(m => m.HandleException(null, null)).Returns(Task.CompletedTask);

			ErrorHandlerMiddleware errorHandlerMiddleware = new ErrorHandlerMiddleware(requestDelegate, errorHandlerMock.Object);
